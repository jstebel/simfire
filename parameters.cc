...
void BoundaryConditions::declare_parameters(ParameterHandler &prm)
{
	prm.enter_subsection("BoundaryConditions");
		prm.declare_entry("Values",    "", Patterns::Map(Patterns::Integer(), Patterns::Anything()));
		prm.declare_entry("Tractions", "", Patterns::Map(Patterns::Integer(), Patterns::Anything()));
	prm.leave_subsection();
}
...
{
	prm.enter_subsection("BoundaryConditions");
		bc = string_to_map_int_string(prm.get("Values"), ',', ':');
		traction = string_to_map_int_string(prm.get("Tractions"), ',', ':');
	prm.leave_subsection();
}
...
void IO::declare_parameters(ParameterHandler &prm)
{
	prm.enter_subsection("InputOutput");
		...
		prm.declare_entry("Mesh for fibers", "", Patterns::FileName(), "Mesh file for fibers (relevant if Materials/Fiber/Use 1d mesh = true)");
...
void IO::parse_parameters(ParameterHandler &prm)
{
	prm.enter_subsection("InputOutput");
		...
		mesh1d_file = prm.get("Mesh for fibers");
